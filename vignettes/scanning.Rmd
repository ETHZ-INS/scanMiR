...
## Basic Aggregation
Given a `GRanges` or data.frame of matches as returned by `findSeedMatches`, aggregation can be performed by the function `aggregateSites`:
```{r}
agg_matches <- aggregateSites(kd_matches)
```
...
### Parallel Processing
Like `findSeedMatches` also `aggregateSites` supports multithreading by passing `BP = MulticoreParam(ncores)`.
