...
default parameter values for aggregation that have been determined by fitting and validating 
the model using several experimental data sets:
```{r}
scanMiR:::.defaultAggParams()
```
The parameters `p3`, `coef_utr`, and `coef_orf` are additionally used to account for the influence of 3' pairing, and the lengths of the UTR and ORF regions, respectively. To disable these features, they can simply be set to zero. `keepSiteInfo` lets you choose whether the site count table should be returned. 
